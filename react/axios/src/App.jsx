...
export default function App() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      // axios는 자동으로 json 파싱을 해주므로 response.data로 바로 접근가능
      ...
      if (err.response) {
        ...
        // 요청은 전송됐으나 응답을 받지 못했을때
        setError("서버로부터 응답이 없습니다.");
        // 서버죽음(응답없음), 인터넷연결 문제, 요청 타임아웃
      } else {
        // 요청 설정 중 오류 발생
        setError(`요청 오류 : ${err.message}`);
        // 잘못된 url (ht%%ps://) 문자 ,axios 실행문제, 코드 문법문제
      }
    } finally {
      setLoading(false);
    }
    // 오류속성
    // err.response.status = http상태코드
    // err.response.data 서버가 전달한 에러메세지
    // err.request 서버로 보낸 실제 요청 객체
    // err.message 자바스크립트 기본 오류 메세지
  };

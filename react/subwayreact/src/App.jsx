...
// 아이템 데이터->각 아이템 name image description(문자열 배열)
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        description: [
          "7시간 저온 훈연한 미국 정통 스타일의",
          "리얼 바비큐 풀드포크는 오직 써브웨이에서",
        ],
      },
      {
        name: "스테이크&치즈",
...
export default function App() {
  ...
  // activeMenuIndex : 활성화 된 탭메뉴 카테고리 인덱스

  const sliderSetting = {
    dots: true, //하단의 점 네비게이션 표시
    dotsClass: "dot-list", //커스텀 dots클래스명
      ...
      <section id="section02">
        <div className="s02_wrap">
            ...
            <ul className="mt_right">
              {menuCategories.map((category, index) => (
                <li
                  key={category.label}
                  className={`${
                    index === menuCategories.length - 1 ? "last" : ""
                  } ${activeMenuIndex === index ? "on" : ""}`.trim()}
                >
                  <a
                    href="#"
                    onClick={(event) => {
          ...
          <div className="menu_wrap">
            {menuCategories.map((category, index) => (
              <div
                key={`menu-${category.label}`}
                className="menu"
                style={{
                  display: activeMenuIndex === index ? "block" : "none",
                }}
              >
                <ul className="cf">

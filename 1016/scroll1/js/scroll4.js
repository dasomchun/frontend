$(function(){
    ...
    $(window).scroll(function(){
        ...
        //     $("#float_div ul li").eq(0).addClass("on"); // 같이 적용도 가능
        // }
        ...
        $(".container > div").each(function(i, e1){
             if(sct >= $(".container > div").eq(i).offset().top){
            $("nav ul li").removeClass("on");
            $("nav ul li").eq(i).addClass("on");
            $("#float_div ul li").removeClass("on");
        ...
        $(".container >div").mousewheel(function(event,d){
            ...
            if(d>0){//휠 위로 감지(+1)
              ...
              let prevVal = $(this).prev().offset().top;
               $("html,body").stop().animate({scrollTop:prevVal},1000,'easeOutBounce');
            }

            //크롬에서 jquery.easing.1.3.js" 검색
      ...
      // 처음에는 pop 변수도 없고 no 도 없음
        ...
        function closePop(){
            if(chk.is(":checked")){ //chk에 체크가 돼있으면
                ...
                // 쿠키 popup이 none을 가지고 3일의 기한을 가진다.
            }
...
// }
 ...
 //크롬에서 jquery.easing.1.3.js" 검색해서 원하는 효과 가능